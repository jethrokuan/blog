...
anything for a while, so some user feedback here would be help.

A popular extension, ~org-roam-server~, is not yet compatible with v2, but I'm
pleased to see that there are people stepping up to build it. It shouldn't take
too long to get to feature parity. There's a lot more that can be done with the
Org-roam buffer that I have yet to explore.
