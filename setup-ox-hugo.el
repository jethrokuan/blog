...
(defvar ox-hugo-test-setup-verbose t
...
(defvar ox-hugo-tmp-dir (let ((dir (cond ((getenv "OX_HUGO_TMP_DIR")
                                          (file-name-as-directory (getenv "OX_HUGO_TMP_DIR")))
                                         (t
...
(defvar ox-hugo-site-git-root (format "%sox-hugo/" ox-hugo-tmp-dir))

(unless (file-exists-p ox-hugo-site-git-root)
  (shell-command (format "git clone https://github.com/jethrokuan/ox-hugo %s" ox-hugo-site-git-root)))
